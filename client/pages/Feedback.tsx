...
export default function Feedback() {
  ...
  const [sentiment, setSentiment] = useState<"pos" | "neu" | "neg" | null>(
    null,
  );
    ...
    return (
      <div className="container py-8 space-y-6">
        ...
        <div className="grid gap-6 lg:grid-cols-3">
          <div className="rounded-2xl border bg-card p-5 shadow-sm lg:col-span-2">
            <div className="font-semibold mb-2">
              Average Ratings by Department ({region})
            </div>
            <div className="h-72">
        ...
        <p className="text-xs text-muted-foreground">
          Admins can use this to benchmark departments and track satisfaction
          trends.
        </p>
      </div>
    );
        ...
        <div className="lg:col-span-2 rounded-2xl border bg-card p-6 shadow-sm">
            ...
            <div className="mt-2 flex gap-2">
              {[1, 2, 3, 4, 5].map((n) => (
                <button
                  key={n}
                  onClick={() => setRating(n)}
                  className={`p-2 rounded-md border ${rating >= n ? "bg-amber-400 text-white" : "bg-background"}`}
                  aria-label={`Rate ${n}`}
                >
          ...
          <div className="mt-4">
            ...
            <div className="mt-2 flex gap-2">
              <button
                ...
                className={`p-2 rounded-md border ${sentiment === "pos" ? "bg-emerald-500 text-white" : "bg-background"}`}
              >
                <Smile className="h-5 w-5" />
              </button>
              <button
                onClick={() => setSentiment("neu")}
                className={`p-2 rounded-md border ${sentiment === "neu" ? "bg-saffron text-white" : "bg-background"}`}
              >
                <Meh className="h-5 w-5" />
              </button>
              <button
                onClick={() => setSentiment("neg")}
                className={`p-2 rounded-md border ${sentiment === "neg" ? "bg-rose-500 text-white" : "bg-background"}`}
              >
          ...
          <div className="mt-4">
            ...
            <textarea
              ...
              placeholder="Describe your experience (department, ward, resolution quality)"
            />
          </div>
                ...
                <li key={item.id} className="rounded-md border p-3">
                  ...
                  <div className="mt-2 flex gap-2 text-xs">
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      href={`https://wa.me/?text=${encodeURIComponent(item.comment || "Feedback")}`}
                    >
